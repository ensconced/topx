...
describe("pipeline", () => {
  ...
  it("propagates updates along a diamond-shaped graph", () => {
    const a = pipeline.node("a");
    const b = pipeline.node("b", (aState) => aState + 1);
    const c = pipeline.node("c", (aState) => aState + 2);
    const spy = jest.fn();
  ...
  it("throws an error if there is a dependency cycle", () => {
    const a = pipeline.node("a", (bState) => bState + 1);
    const b = pipeline.node("b", (cState) => cState + 2);
    const c = pipeline.node("c", (aState) => aState + 3);
    const dependencies = new Map();
